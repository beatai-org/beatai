...
![一部卡通手机屏幕，上面热门 App 用发光的线连到一台 1951 年的复古电脑，那台电脑正在跑 KNN](./images/knn-distance-metrics-choosing-k-modern-ai-explained/01.webp)
*For You 页面、Discover Weekly、ChatGPT memory，都是同一个算法。1951 年。*
...
![一只卡通树懒一边读一张索引卡，一边慢吞吞地把它归档到代表 KNN 训练的标签柜里](./images/knn-distance-metrics-choosing-k-modern-ai-explained/04.webp)
*这就是它的训练过程。它在学习。看看它学得多认真。*
...
![风格化插画：手机上的各种 App 是一栋栋楼，用发光的最近邻连线连成一片星座](./images/knn-distance-metrics-choosing-k-modern-ai-explained/05.webp)
*这是你的手机。每一条发光的线都是同一个算法。*
...
![同一对点的并排两图，一边是 Euclidean 的斜线，一边是 Manhattan 的阶梯式出租车路径](./images/knn-distance-metrics-choosing-k-modern-ai-explained/06.webp)
*同样的两个点，两种度量。一个模型决策之后，你的准确率涨了 8% 或者掉了 12%。*
...
![五个卡通运动员形象代表五种距离度量，正在向起跑线另一端那颗发光的查询星冲刺](./images/knn-distance-metrics-choosing-k-modern-ai-explained/08.webp)
*五种距离度量走进了一个模型。最后只有一个拿到了工作。*
...
![三联散点图，对同一个数据集分别展示 K=1、K=7、K=30 下的 KNN 决策边界](./images/knn-distance-metrics-choosing-k-modern-ai-explained/09.webp)
*同样的数据，同样的算法，三个 K 值，三个完全不一样的模型。*
...
![折线图：训练误差和验证误差对 K 的曲线，最优 K 标在验证误差的拐肘处](./images/knn-distance-metrics-choosing-k-modern-ai-explained/10.webp)
*粉色那条线是你的训练集在对你撒谎。蓝色那条线说的是真话。听蓝色那条的。*
...
![卡通版的金发姑娘站在三碗粥前，分别代表"K 太小"、"K 刚好"、"K 太大"](./images/knn-distance-metrics-choosing-k-modern-ai-explained/11.webp)
*KNN 调参，就是更糟糕职业路径上的"粥温工程"。*
...
![折线图：随着维度增加，最大距离与最小距离的比值趋近 1](./images/knn-distance-metrics-choosing-k-modern-ai-explained/12.webp)
*这就是你不假思索地往模型里加特征时，KNN 直觉会掉下去的那道悬崖。*
...
![一个卡通科学家用缩小射线把巨象一样大的特征和蚂蚁一样小的特征调成同样大小，公平比较](./images/knn-distance-metrics-choosing-k-modern-ai-explained/13.webp)
*标准化就是给特征讲公平。让它们以同样的音量吵架。*
...
![卡通开发者坐在发光的终端前，旁边漂着一朵 Iris 花和写着 sepal、petal 的小尺子](./images/knn-distance-metrics-choosing-k-modern-ai-explained/14.webp)
*我代码第一次干净跑通、并且花儿们配合得很好的那一刻。*
...
![卡通场景：最近的那个邻居拿着大喇叭，比远处的邻居喊得响——加权 KNN 的形象化](./images/knn-distance-metrics-choosing-k-modern-ai-explained/15.webp)
*五个邻居，其中一个有大喇叭。猜谁会赢。*
...
![风格化卡通图书馆，发光的"书本向量"通过图连线相连，代表近似最近邻搜索](./images/knn-distance-metrics-choosing-k-modern-ai-explained/16.webp)
*向量数据库这个数十亿美金的行业，是在一个 1951 年的算法稍微更快一点的版本上下的赌注。孩子们过得还不错。*
...
![卡通 LLM 角色在读由 KNN 图书管理员角色挑出的书页，演示 RAG 的检索流水线](./images/knn-distance-metrics-choosing-k-modern-ai-explained/17.webp)
*LLM 是大脑，KNN 是图书管理员。两个都按时拿工资。*
...
![卡通医生拿着听诊器给一个小小的 KNN 机器人病人看病，手里捏着两张处方单](./images/knn-distance-metrics-choosing-k-modern-ai-explained/19.webp)
*有时候正确的判断是"用另一个算法"。模型也是有感情的。*
...
![卡通笔记本，几页折了角，每一页上画着一个关于 KNN 的小教训](./images/knn-distance-metrics-choosing-k-modern-ai-explained/20.webp)
*一份没人要的田野日志。欢迎使用。*
...
![卡通工程师站在分层金字塔底端，最下一层标着 KNN，最上一层标着 AI apps](./images/knn-distance-metrics-choosing-k-modern-ai-explained/21.webp)
*每一个花哨的 AI 工具都坐在这座金字塔上。底层那块石头，是彩色电视出现之前就刻好的。*
...
![卡通开发者关上贴着 K 形贴纸的笔记本，夕阳落在风格化的手机 App 天际线上](./images/knn-distance-metrics-choosing-k-modern-ai-explained/22.webp)
*发推。出去走走。打开你最喜欢的 App，看一个 1951 年的算法替你决定今晚怎么过。*
