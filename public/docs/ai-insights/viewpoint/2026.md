...
#### 2026年3月
- [Agent/Skills/Teams 架构演进过程及技术选型之道](https://mp.weixin.qq.com/s/Z8JYgxUdHSLo4ywgyt4ljg) 一篇从 Single Agent 到 Agent Teams 的完整演化复盘，重点不在“新名词”，而在如何按场景做架构取舍。
- [Agent/Skills/Teams 架构演进过程及技术选型之道（站内高保真辣评版，霍华德口吻）](/docs/ai-insights/viewpoint/agent-skills-teams-architecture-evolution.md) 在保留原文结构、数据链和配图位置的前提下，用工程化、吐槽式口吻重写核心判断，强调“先基线、后并行、按场景升级”的选型节奏。
- [Your Claude setup rots over time. Detox it in 60 seconds (with this self-audit prompt):](https://x.com/itsolelehmann/status/2036533756572639611) 一条非常实用的长文，核心观点很反直觉：Claude 输出变差时，不一定该继续加规则，更可能该先删掉已经腐烂的那部分配置。
- [你的 Claude 配置会随着时间变质：用这条自检提示词，60 秒排毒（站内中文翻译版）](/docs/ai-insights/viewpoint/claude-self-audit-prompt.md) 真正让 Claude 越用越乱的，不一定是模型，而是那堆没人再回头清理的旧规则；这篇最有价值的地方，就是给出了一条能直接动手清理配置的 prompt。
