...
> 辣评：如果一个团队连问题边界都没定义清楚，却先讨论“要不要上多智能体”，那不是架构设计，是昂贵的行为艺术。Bazinga.
...
> 辣评：把术语层层叠叠剥开后，结论朴素到近乎残忍：你只是换着花样给模型补“知识注入”和“长期记忆”。其余大多是包装。
...
> 辣评：单智能体不是“低配版”，而是基线实验。跳过基线直接上群体协作，等于在没有对照组的情况下宣布论文显著，科学性约等于零。
...
面对这一困境，行业普遍采用的解决方案是引入RAG（检索增强生成）。
...
> 辣评：多智能体真正昂贵的不是并行本身，而是通信损耗与上下文断裂。只算吞吐不算协调成本，这账做得像我绝不会通过的作业。

面对单Agent在海量知识注入和复杂场景处理上的局限，Multi-Agent架构（多智能体）应运而生。这不仅是Agent数量的堆叠，更是质量的飞跃。以我们阿里云客户服务领域云小二Aivis的实践为例，通过构建Planner（规划者）、Reasoner（推理者）、Executor（执行者）等角色协同的架构，我们将复杂的宏观问题拆解为微观的子任务，由不同的Agent各司其职。
...
> 辣评：Skills 的价值在于把“群聊式协作”退回到“单体可控执行”。能由一个主 Agent 读手册完成的事，没必要开三十个会。
...
> 辣评：Agent Teams 适合探索未知，不适合当 KPI 装饰。没有中心化校验的并行试错，通常只会得到更贵、更一致、也更错误的答案。
...
> 辣评：这部分终于把“我觉得”替换成了“我验证过”。从信仰驱动到实验驱动，这才是工程，不是许愿。
...
> 辣评：从 P0 到 P3 的顺序非常理性：先用最小复杂度拿到可行解，再用复杂度预算换上限。先把刀磨快，再谈手术机器人。
...
> 辣评：Less structure, more intelligence 不是励志贴纸，而是约束条件。结构加得越晚越好，证据越硬越好。是的，这才配叫方法论。
