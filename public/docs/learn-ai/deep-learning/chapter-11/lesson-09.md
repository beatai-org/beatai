...
这种需要对图片上每个像素都赋予语义信息的任务叫做语义分割（semantic segmentation）。 ![1065.png](../imgs/1065.png)
...
如果步长为2，则计算过程如下图： ![1067.png](../imgs/1067.png)

同样，转置卷积的卷积核参数是在训练过程中学到的。通过学习，它知道如何上采样能保持图像的语义信息。

### 11.9.3 U-Net原理

![1068.png](../imgs/1068.png) U-Net的网络结构图如上图所示，因为其像字母U，而命名为U-Net。 U-Net 的网络主要分为两个部分： 编码器（Encoder）——下采样路径 解码器（Decoder）——上采样路径 此外还有一条贯穿左右的 跳跃连接（Skip Connections），它是 U-Net 的核心创新之一。
