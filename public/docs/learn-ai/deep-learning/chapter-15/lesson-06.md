...
在上一章里，我们用RNN训练了一个英文翻译中文的模型，这一节我们改为Transformer模型。 完整的训练代码可以从github下载[train.py](https://github.com/RethinkFun/DeepLearning/blob/master/chapter15/train.py)。它依赖于[transformer.py](https://github.com/RethinkFun/DeepLearning/blob/master/chapter15/transformer.py)。
...
同时，你也可以测评它的BLEU score。具体代码和上一章的代码一样, [BLEU\_score.py](https://github.com/RethinkFun/DeepLearning/blob/master/chapter15/BLEU_score.py)。它依赖于上边的inference.py。通过运行BLEU score，你也可以发现模型性能比RNN的翻译模型要提升不少。
