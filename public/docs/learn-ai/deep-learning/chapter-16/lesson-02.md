...
所以最终BERT的输入如下： ![1606.png](../imgs/1606.png) 上图中最上边的Input是原始输入，我们给所有的输入前增加一个特殊的\[cls\] token，它是一个占位符，将来在BERT Encoder Block最后一层第一个位置的输出向量，就代表了整个输入的全局信息。利用这个信息，加一个分类头就可以对序列（单序列或者多序列）进行分类。我们发现Input中间还有一个\[SEP\] token，它就是序列的分隔符，用来隔开两个序列的。通过这个token，让模型知道这个token的前后属于不同的序列。
